...
public class IncreaseDecrease extends Action {
    ...
    public IncreaseDecrease(PRDAction action) {
        super(Actions.DECREASE, action.getEntity(), action.getPRDSecondaryEntity());
    ...
    public void Act(World world, Entity entity, Entity secondEntity, int currentTicks, List<Runnable> actionsForEndTick) {
        ...
        String expression = ExpressionParser.eval(this.by, world, entity, secondEntity,currentTicks);
        ...
        p.setValue(Result(p.getValue(), expression),currentTicks);
    }
