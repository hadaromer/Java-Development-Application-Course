...
public class Condition implements Serializable {
    ...
    public boolean Check(World world, Entity entity, Entity secondEntity, int currentTicks) {
        if (this.singularity == Singularity.SINGLE) {
            return CheckSingle(world, entity, secondEntity, currentTicks);
        } else {
            if (logical == Logical.AND) {
                return conditions.stream().allMatch(condition -> condition.Check(world, entity, secondEntity, currentTicks));
            } else
                return conditions.stream().anyMatch(condition -> condition.Check(world, entity, secondEntity, currentTicks));
        }
    }

    private boolean CheckSingle(World world, Entity entity, Entity secondEntity, int currentTicks) {
        String expression = ExpressionParser.eval(this.value, world, entity, secondEntity, currentTicks);
        String entityValue = ExpressionParser.eval(this.property,world,entity,secondEntity,currentTicks);
        String entityValueType = Utils.ValueType(entityValue);
